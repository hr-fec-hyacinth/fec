...
const Expanded = ({ updateExpand, styling, imageIndex, imageList, updateStyling, updateImageIndex }) => {
  ...
  return (
    <div className='fixed top-0 left-0 z-30 w-full h-full bg-contain backdrop-blur bg-black/30 flex justify-evenly items-center'>
      ...
      <img src={imageList[imageIndex]} className='max-w-screen max-h-full object-contain bg-no-repeat bg-center select-none cursor-cell' onClick={e => {
        updateZoomed(true);
        updateZoomEvent(e);
      }}/>
        <MdClose className='hover:text-gray-500 text-white right-4 top-4 absolute text-7xl sm:text-2xl cursor-default z-30' onClick={e => { updateExpand(false) }} />
        ...
        {imageIndex !== 0 && <BsFillArrowLeftCircleFill className='hidden sm:block hover:text-gray-500 text-2xl text-white top-1/2 left-24 absolute z-20 cursor-default' onClick={previous} />}
        ...
        {imageIndex !== imageList.length - 1 && <BsFillArrowRightCircleFill className='hidden sm:block hover:text-gray-500 text-2xl text-white top-1/2 right-24 absolute z-10 cursor-default' onClick={next} />}

        <div className='absolute bottom-4 flex'>
          {imageList.map((image, i) => {
            if (i === imageIndex) {
              return (<VscCircleLargeFilled key={i} className='sm:text-base text-2xl rounded-full text-blue-400 border-2 border-white mx-1 cursor-default' />);
            }
            return (<VscCircleLargeFilled key={i} className='sm:text-base text-2xl text-white mx-1 cursor-default' onClick={e => {
              updateImageIndex(i);
            }} />);
          })}
      </div>
